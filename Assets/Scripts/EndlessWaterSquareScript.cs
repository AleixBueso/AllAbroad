...
public class EndlessWaterSquareScript : MonoBehaviour
{
    ...
    List<WaterSquareScript> waterSquares = new List<WaterSquareScript>();
    ...
    void UpdateWaterWithThreadPooling(object state)
    {
        ...
        for (int i = 0; i < waterSquares.Count; i++)
        {
            ...
            for (int j = 0; j < vertices.Length; j++)
            {
                ...
                vertexPos.y = WaterControllerScript.current.GetWaveYPos(vertexPosGlobal, secondsSinceStart);
    ...
    void AddWaterPlane(float xCoord, float zCoord, float yPos, float squareWidth, float spacing)
    {
        ...
        WaterSquareScript newWaterSquare = new WaterSquareScript(waterPlane, squareWidth, spacing);
